...
void SLIC_Algorithm_AoS_Parallel::iteration(){
    ...
    const int cols = this->image_lab.cols;
    const int grid_w = cols / S;
    // Calcolo questo fuori per evitare divisioni ripetute nei check
        ...
        #pragma omp  for schedule(runtime)
            for (int y = 0; y < rows; y++) {
                ...
                for (int x = 0; x < cols; x++) {
                    ...
                    float pos_x = pxls[idx].x;
                    float pos_y = pxls[idx].y;

                    double min_distance = DBL_MAX;
                    int best_k = pxls[idx].label;
                    bool changed = false;
                    ...
                    for (int ny = -1; ny <= 1; ny++) {
                        ...
                        for (int nx = -1; nx <= 1; nx++) {
                            int kx = grid_x + nx;
                            if (kx >= 0 && kx < grid_w && ky >= 0 && ky < grid_h) {
                ...
                for (int tx = 0; tx < cols; tx += TILE_SIZE) {
                    ...
                    for (int y = ty; y < y_end; y++) {
                        ...
                        for (int x = tx; x < x_end; x++) {
                            ...
                            double min_distance = pxls[idx].distance;
                            int best_k = pxls[idx].label;
                            bool changed = false;
                                ...
                                for (int nx = -1; nx <= 1; nx++) {
                                    ...
                                    if (kx >= 0 && kx < grid_w &&
                                        ...
                                        if (k >= 0 && k < K) {
                                            ...
                                            if (abs(spxls[k].centroid_x - x) < 2 * S &&
                                                abs(spxls[k].centroid_y - y) < 2 * S) {
                                                double d = distance_SLIC(spxls[k].val_L, spxls[k].val_a, spxls[k].val_b, spxls[k].centroid_x,
                                                    spxls[k].centroid_y, val_L, val_A,val_B,pos_x,pos_y, S,m);
